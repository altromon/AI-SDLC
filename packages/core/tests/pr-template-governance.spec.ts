...
describe('PR Template and Weak Points Governance Suite', () => {
  ...
  it('should have .github/PULL_REQUEST_TEMPLATE.md with all mandatory governance sections', () => {
    ...
    expect(content).toMatch(/Plan Execution Matrix|Matriz de Ejecución del Plan/i);
    expect(content).toMatch(/\[X\]/);
    expect(content).toMatch(/\[O\]/);
    expect(content).toMatch(/each and every|todos y cada uno/i);
    expect(content).toMatch(/mandatory justification|justificación obligatoria/i);

    // Section 3: Weak Points Hotspots & AI Assumptions
    expect(content).toMatch(/Weak Points Map|Mapa de Puntos Débiles/i);
    expect(content).toMatch(/Complexity Hotspots|Hotspots de Complejidad/i);
    expect(content).toMatch(/Edge Cases|Casos Límite/i);
    expect(content).toMatch(/AI Assumptions|Asunciones de la IA/i);

    // Section 4: Security STRIDE
    expect(content).toMatch(/Attack Surface|Superficie de Ataque/i);
  ...
  it('should document Tech Lead blocking criteria and X/O governance in process/01_governance_and_roles.md', () => {
    ...
    expect(content).toMatch(/Plan Execution Matrix|Matriz de Ejecución del Plan/i);
    expect(content).toMatch(/each and every|todos y cada uno/i);
    expect(content).toMatch(/block|bloque/i);
    expect(content).toMatch(/AI Assumptions|Asunciones de la IA/i);
    expect(content).toMatch(/Complexity Hotspots|Hotspots de Complejidad/i);
  });
